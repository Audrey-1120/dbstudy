...
SELECT E.EMP_NO, E.NAME, E.DEPART, D.DEPT_NAME
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E
    ON D.DEPT_NO = E.DEPART;  -- 테이블과 칼럼의 순서를 맞추는 것이 좋다.
...
/*
    ...
    2. 해당 테이블이 왼쪽에 있으면 왼쪽 외부 조인이고 오른쪽에 있으면 오른쪽 외부 조인.
    3. ANSI 문법
        1) 왼쪽 외부 조인
        SELECT 조회할 칼럼, ...
...
-- 1. 모든 사원들의 사원번호, 사원명, 부서명 조회하시오.
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM DEPARTMENT_T D RIGHT JOIN EMPLOYEE_T E -- 홍길동 회장은 오른쪽(EMPLOYEE_T)에 있기 때문에 RIGHT JOIN.
    ON D.DEPT_NO = E.DEPART;
...
SELECT D.DEPT_NAME, COUNT(E.EMP_NO)
  FROM DEPARTMENT_T D LEFT JOIN EMPLOYEE_T E --총무부와 인사부는 없음. 이 데이터도 확인하려면 왼쪽부서 테이블에 있기 때문에.. LEFT_JOIN
    ON D.DEPT_NO = E.DEPART
  GROUP BY D.DEPT_NAME;
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, J.JOB_TITLE, J.MAX_SALARY, E.SALARY
...
SELECT DISTINCT C.COUNTRY_NAME, L.CITY
...
-- 같은 부서의 사원만 조인하기 위해서 부서 번호로 조인 조건을 생성함
