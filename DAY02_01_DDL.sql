...
CREATE TABLE CUSTOMER_T(
    ...
    CUST_AGE NUMBER(3) CHECK(CUST_AGE >= 0 AND CUST_aGE <= 120),
                    -- CHECK(CUST_AGE BETWEEN 0 AND 120), 동일한 방법임.
...
/*
    ...
    4. 칼럼 이름 변경
    - ALTER TABLE 테이블명 RENAME COLUMN 기존칼럼 TO 신규칼럼
    5. 테이블 이름 변경
...
ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --ALTER의 경우 NULL로 바꿀거면 NULL넣어주어야 함.
...
/*
    ...
    2. FK
        1) 추가 : ALTER TABLE 자식 테이블명 ADD CONSTRAINT 제약조건명 FOREIGN KEY (칼럼명) REFERENCES 부모테이블명(참조할 칼럼명) [옵션]
        2) 삭제 : ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건명
        3) 비활성화 : ALTER TABLE 테이블명 DISABLE CONSTRAINT 제약조건
